...
/*	【动画封装】
	...
	还可以加上时间间隔，不加就是使用默认值，加上时间值时，要给回调函数用null来占位。
*/
...
//		【事件的委托，对象传参的方式】
//用法：   GOev({id:"box",eventType:"click",callback:function(){alert(this.innerHTML);},type:"li"});
...
//	【HTML5  local 和 session 的封装】
//	setCache(key,value,true)	参数后面加了true就是localStorage，没加默认是sessionStorage		
		
		function setCache(key,value){
			var args = arguments; 
			Storage = args[2] ? localStorage:sessionStorage;
			Storage.setItem("GQ"+key,value);
		};
			
		function getCache(key){
			var args = arguments; 
			Storage = args[1] ? localStorage:sessionStorage;
			 return Storage.getItem("GQ"+key);
		};
	
		function removeCache(key){
			var args = arguments; 
			Storage = args[1] ? localStorage:sessionStorage;
			Storage.removeItem("GQ"+key);
		};
